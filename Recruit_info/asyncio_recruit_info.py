...
collection = pymongo.MongoClient(host='127.0.0.1', port=27017)['Falonie']['app_positions']
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, proxy=None) as response:
            ...
            for _ in sel.xpath('//div[@class="jtag inbox"]/div[@class="t1"]'):
                ...
                item['job_description'] = re.sub(r'[\r\t\xa0\0x80\u3000 ]', '', ''.join(str(i).strip() for i in job_description))
                item['url']=url
...
def read_mongodb():
    collection_filter_duplicates = pymongo.MongoClient(host='127.0.0.1', port=27017)['Falonie']['app_urls']
...
def read_excel(file):
    with xlrd.open_workbook(file) as data_:
        ...
        company_list_ = [table.row_values(rownum)[0] for rownum in range(7123, table.nrows)]
